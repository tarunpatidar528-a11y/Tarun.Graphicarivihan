...
function loadState(){try{const saved=localStorage.getItem(STORAGE_KEY);if(saved)window.state=JSON.parse(saved)}catch(e){}
...
state.folders=DEFAULT_FOLDERS.concat(state.thumbnailSubfolders.map(x=>'Thumbnails/'+x));
if(!state.currentFolder||!state.folders.includes(state.currentFolder))state.currentFolder='Thumbnails'}
function saveState(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify(state))}catch(e){alert('Browser storage is full. Please delete old images or use smaller files.')}}
